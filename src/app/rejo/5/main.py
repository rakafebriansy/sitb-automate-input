...
# ---------- Config ----------
FILE_NAME = "data/rejo/5/rawat_jalan_clean.xlsx"
STATE_FILE = "data/rejo/5/state.json"
ITERATE = None
...
if not os.path.exists(FILE_NAME):
    ...
    for idx in range(start_index, iterate_count):
        ...
        try:
            ...
            bb = float(bb_raw) if not pd.isna(bb_raw) and float(bb_raw) > 0 else None
            tb = float(tb_raw) if not pd.isna(tb_raw) and float(tb_raw) > 0 else None

            # Skip row if invalid or unrealistic (e.g., TB < 100 or BB < 20)
            if bb is None or tb is None or tb < 100 or bb < 20:
                print(f"[⚠️] Skipping row {idx + 1} — invalid BB/TB ({bb_raw}, {tb_raw})")
                continue
